...
class encoder(nn.Module):
    ...
    def forward(self, x_input, encoder_hidden_states=None):
        ...
        # print(f"x_input.view shape = {x_input.view(x_input.shape[0], x_input.shape[1], self.input_size).shape}")
        lstm_out, self.hidden = self.model(x_input.view(x_input.shape[0], x_input.shape[1], self.input_size), encoder_hidden_states)
...
class MCL(nn.Module):
    ...
    def __init__(self, input_sizes, 
        ...
        self.input_sizes = input_sizes # dict: we will have different input sizes for different models
        self.hidden_size = hidden_size
        self.num_layers = num_layers # dict: let's have a dict. assuming we have different number of layers for each model
        self.model_type = model_type
        self.output_size = output_size # assuming output size of each mode would be the same
        self.device = device
        ...
        for i in range(num_models):
            ...
            # self.models[f'model_{i}'] = self.pretrained_models[i]
            # self.models[f'model_{i}'].device = self.device
            # self.models[f'model_{i}'].utils = utils
            self.models[f'model_{i}'] = seq2seq(input_size=self.input_sizes[i], 
    ...
    def train_MCL(self, 
        ...
        if process_to_unfreeze is not None:
            self.model_ft = self.models['model_'+str(self.model_to_num_map[process_to_unfreeze])]
        
        n_batches = int(math.ceil(X_train.shape[0] / config.batch_size))
        optimizer = optim.Adam(self.model_ft.parameters(), lr=config.learning_rate, weight_decay=config.weight_decay)
        ...
        wandb.watch(self.model_ft.encoder)
        ...
        for it in range(n_epochs):
            ...
            for input_batch, target_batch in training_generator:
                ...
                # hidden state initialization of encoders in each model
                for i in range(self.num_models):
                    encoder_hidden[i] = self.models['model_'+str(i)].encoder.init_hidden(config.batch_size)
                ...
                encoder_output_1, encoder_hidden_1 = self.models['model_0'].encoder(input_batch)

                encoder_output_2, encoder_hidden_2 = self.models['model_1'].encoder(input_batch[:, :, :self.input_sizes[1]], encoder_hidden_1) # we will have to add the input features for each model
                encoder_output_3, encoder_hidden_3 = self.models['model_2'].encoder(input_batch[:, :, :self.input_sizes[2]], encoder_hidden_2) # we will have to add the input features for each model
                encoder_output_4, encoder_hidden_4 = self.models['model_3'].encoder(input_batch[:, :, :self.input_sizes[3]], encoder_hidden_3) # we will have to add the input features for each model
                encoder_output_5, encoder_hidden_5 = self.models['model_4'].encoder(input_batch[:, :, :self.input_sizes[4]], encoder_hidden_4) # we will have to add the input features for each model
                ...
                # decoder_hidden_3 = encoder_hidden_3
                # decoder_hidden_4 = encoder_hidden_4
                decoder_hidden_5 = encoder_hidden_5
                ...
                if training_prediction == 'recursive':
                    ...
                    for t in range(target_len):
                        ...
                        for i in range(self.num_models):
                            ...
                            # decoder_output_1, decoder_hidden_1 = self.models['model_0'].decoder(decoder_input, decoder_hidden_1)
                            # decoder_output_2, decoder_hidden_2 = self.models['model_1'].decoder(decoder_output_1, decoder_hidden_2)
                            # decoder_output_3, decoder_hidden_3 = self.models['model_2'].decoder(decoder_output_2, decoder_hidden_3)
                            # decoder_output_4, decoder_hidden_4 = self.models['model_3'].decoder(decoder_output_3, decoder_hidden_4)
                            decoder_output_5, decoder_hidden_5 = self.models['model_4'].decoder(decoder_input, decoder_hidden_5)

                            # decoder_output, decoder_hidden = self.decoder(decoder_input, decoder_hidden)
                            outputs[:,t,:] = decoder_output_5
                            decoder_input = decoder_output_5
                ...
                del input_batch
                del target_batch
                del outputs, encoder_output_1, encoder_output_2, encoder_output_3, encoder_output_4, encoder_output_5
                del decoder_output_5, decoder_hidden_5
            ...
            if it % config.eval_freq == 0:
                with torch.no_grad():
                    test_eval_dict = self.evaluate_batch(generator=validation_generator)#, X_test=X_test, Y_test=Y_test)
                    train_eval_dict = self.evaluate_batch(generator=training_generator)#, X_test=X_train, Y_test=Y_train)
        ...
        print("Final eval !!!")
        test_eval_dict = self.evaluate_batch(generator=validation_generator)
        train_eval_dict = self.evaluate_batch(generator=training_generator)
    ...
    def predict_batch(self, target_len, validation_generator):
        '''
        ...
        predictions = []
        targets = []
        for input_batch, target_batch in tqdm(validation_generator):
            input_batch = input_batch.to(self.device)
            
            encoder_output_1, encoder_hidden_1 = self.models['model_0'].encoder(input_batch)
            encoder_output_2, encoder_hidden_2 = self.models['model_1'].encoder(input_batch[:, :, :self.input_sizes[1]], encoder_hidden_1)
            encoder_output_3, encoder_hidden_3 = self.models['model_2'].encoder(input_batch[:, :, :self.input_sizes[2]], encoder_hidden_2) 
            encoder_output_4, encoder_hidden_4 = self.models['model_3'].encoder(input_batch[:, :, :self.input_sizes[3]], encoder_hidden_3) 
            encoder_output_5, encoder_hidden_5 = self.models['model_4'].encoder(input_batch[:, :, :self.input_sizes[4]], encoder_hidden_4)
            ...
            decoder_input = torch.zeros([target_batch.shape[0], target_batch.shape[2]], device=self.device)  # shape: (batch_size, input_size)
            decoder_hidden = encoder_hidden_5

            # decoder_input = torch.zeros(batch_size, self.output_size, device=self.device)  # input_tensor[-1, :, :]%%!
            # decoder_hidden = encoder_hidden

            for t in range(target_len):
                decoder_output, decoder_hidden = self.models['model_4'].decoder(decoder_input, decoder_hidden)
                outputs[:,t,:] = decoder_output
            ...
            del outputs, encoder_output_1, encoder_output_2, encoder_output_3, encoder_output_4, encoder_output_5
            del encoder_hidden_1, encoder_hidden_2, encoder_hidden_3, encoder_hidden_4, encoder_hidden_5
    ...
    def evaluate_batch(self, generator=None, X_test=None, Y_test=None, unnorm=True):
    
        y_pred, y_test = self.predict_batch(self.utils.output_window, validation_generator=generator) 
        ...
        with trange(n_epochs) as tr:
            for it in tr:
                ...
                if it % config.eval_freq == 0:
                    ...
                    test_rmse.append(batch_test_loss.detach())
                    train_rmse.append(batch_train_loss.detach())
