...
The implicit Crank-Nicolson scheme, whichis second-order derivative in space and time, was applied to solve the one-dimensional temperature transport equation for the diffusive transport. Here, we average the response in space between the current and the next time step:
$$
...
$$
Here, we can apply $\alpha = A K_z \frac{\Delta t}{A \Delta z^2}$, transforming the equation to:
$$
...
$$
$$
- \frac{\alpha}{2} T_{i+1}^{n+1} + (1+\alpha)T_{i}^{n+1}- \frac{\alpha}{2}T_{i-1}^{n+1} = R_i^n
$$
